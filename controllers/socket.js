...
const { Either, Right, Left, Maybe, IO } = require('./monads.js');

const createRandom = () => Math.floor(Math.random() * 5 + 5);
const isMid = (socketid, obj) => socketid === obj.mid;

const waitPare = val => {
  return _.isEmpty(State.getWatingPlayer())
    ? Either.left(val)
    : Either.right(val);
};
...
exports.socketConnect = io => socket => {
  ...
  const isExistRoom = _.curry((socketId, req) => {
    return State.isExistRoom(req.roomId)
      ? errorExist('room-isExist', socketId)
      : Maybe.just(req);
  });
  const isNotExistRoom = _.curry((socketId, req) => {
    return !State.isExistRoom(req.roomId)
      ? errorExist('room-isnotExist', socketId)
      : Maybe.just(req);
  });
  const isFullMenber = _.curry((socketId, req) => {
    return State.isFullRoom(req.roomId)
      ? errorExist('room-isFull', socketId)
      : Maybe.just(req);
  });

  const safeIsDuplicate = _.curry((socketId, req) => {
    return State.isDuplicatePlayer(socketId)
      ? errorExist('duplicate-error', socketId)
      : Maybe.just(req);
  });
  ...
  socket.on('disconnect', () => {
    Maybe.fromNullable(stateDisconnect(socket.id)).map(roomEmit(socket.id));
  });
